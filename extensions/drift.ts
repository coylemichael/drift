...
export default function drift(pi: ExtensionAPI) {
  ...
  pi.on("agent_settled", async (_event, ctx) => {
    ...
    // Pi ACP intercepts slash commands before extension commands. Compaction gives the next task a fresh context window without relying on one.
    ctx.compact({
      ...
      onComplete: () => { void pi.sendUserMessage(`drift-continue ${artifact}`); },
      onError: (error) => ctx.ui.notify(`Automatic Drift context reset failed: ${error.message}. The published handoff remains available for manual continuation.`, "error"),
    });
  });
